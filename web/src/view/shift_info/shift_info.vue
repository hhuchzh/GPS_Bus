<template>
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="busId字段">
          <el-input v-model="searchInfo.busId" placeholder="搜索条件" />
        </el-form-item>
    ...
    >
      ...
      <el-table-column label="busId字段" prop="busId" width="120" />
      <el-table-column label="sendTime字段" prop="sendTime" width="120" />
      <el-table-column label="classesId字段" prop="classesId" width="120" />
      <el-table-column label="shiftMiles字段" prop="shiftMiles" width="120" />
      <el-table-column label="按钮组">
        <template #default="scope">
          <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="updateShiftInfo(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="sendTime字段:">
          <el-date-picker v-model="formData.sendTime" type="date" placeholder="选择日期" clearable />
        </el-form-item>
        <el-form-item label="classesId字段:">
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteShiftInfoByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateShiftInfo(row) {
      const res = await findShiftInfo({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteShiftInfo(row) {
      const res = await deleteShiftInfo({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
