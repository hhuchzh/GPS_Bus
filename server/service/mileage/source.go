...
func (s *Source) LoadCheckIn(id uint64, date string) (list interface{}, err error) {
	global.GVA_LOG.Info("load checkin details", zap.String("date", date))
	db := global.GVA_DB.Model(&autocode.CheckinInfo{})
	var checkins []autocode.CheckinInfo
	db = db.Where("`classes_id` = ?", id)
	db = db.Where("`checkin_date` = ?", date)
	err = db.Find(&checkins).Error
	return checkins, err
}
